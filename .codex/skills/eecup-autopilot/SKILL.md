...
Git 是本工作流的恢复点和实验隔离机制。启动、工作块结束和阶段关卡结束时都要检查 Git 状态。
...
`model_options.md` 必须区分基准模型和候选路线：基准模型不占用候选路线名额；候选路线至少 3 个；路线执行方式必须写清是单路线推进还是多路线分支比较。多路线分支比较只有在用户确认后开启，且必须先完成统一基准模型。
