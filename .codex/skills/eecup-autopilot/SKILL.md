...
建模代码阶段不得只输出指标表。原则上每个小问至少准备：

- 1 张数据理解类图；
- 1 张模型或算法流程图；
- 1 张结果对比类图；
- 1 张验证分析类图，若该小问涉及敏感性、鲁棒性或误差分析。

图表必须服务论文结论，而不是装饰版面。所有图表必须能追溯到脚本和数据，不允许手工改图后直接写入论文结论，也不允许为了图表丰富而编造结果。

图表生成脚本优先放入 `02_src/`；输出保存到 `03_results/figures/` 或 `03_results/models/<model_id>/figures/`。论文中使用的图必须在 `04_paper/paper_draft.md` 中标注来源路径、插入位置和支撑的结论。

数据图、结果图和流程图优先由代码生成。正式建模结果图、流程图、算法图不得用 AI 生成图替代；`imagegen` 只在需要非数据型示意背景、封面视觉或透明装饰素材时考虑。
...
- `references/modeling-playbook.md`：建模、验证和建模图表产出要点。
- `references/model-recovery.md`：模型异常诊断与恢复方案。
- `references/paper-playbook.md`：Markdown 论文初稿、图表美学、流程图插入和论文质量门槛。
- `references/problem-analysis-framework-chart.md`：`2. 问题分析` 总体框架图的版式、内容和插入规范。
